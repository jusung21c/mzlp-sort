...
import MzlpSortView from './mzlp-sort-view';
...
export default {
  ...
  activate(state) {
    this.mzlpSortView = new MzlpSortView(state.mzlpSortViewState);
    this.modalPanel = atom.workspace.addModalPanel({
      item: this.mzlpSortView.getElement(),
      visible: false
    });
  ...
  deactivate() {
    ...
    this.mzlpSortView.destroy();
  },

  serialize() {
    return {
      mzlpSortViewState: this.mzlpSortView.serialize()
    };
  },
  ...
  toggle() {
    ...
    let array = [];
    let selected = this.editor.getSelectedBufferRange();
    ...
    for(let i in array){
        this.editor.insertText(array[i].item+"\n\n");
    }
    alert(array.length + "개 요소 정렬 완료")
  }
,
  getItem(regex,range){
      this.editor.scanInBufferRange(regex,range,function(obj){
          return {"content":obj.match[0],"key":obj.matchText};
      });
  },
  getRange(start,end){
      let temp = [[start.row,start.column],[end.row,end.column]];
      return temp;
  },
  addItems(range,text){
    let obj;
